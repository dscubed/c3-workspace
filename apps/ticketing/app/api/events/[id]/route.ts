...
import { generateUniqueSlug } from "@/lib/api/slugUtils";
...
export async function PATCH(
  ...
  try {
    ...
    if (groups.includes("event")) {
      ...
      /* Auto-generate slug from name if not yet set */
      if ("name" in body && body.name && !existing.url_slug) {
        try {
          const newSlug = await generateUniqueSlug(body.name, eventId);
          if (newSlug) {
            await supabaseAdmin
              .from("events")
          ...
          /* non-critical — slug generation failure should not break the save */
        }
      }
    }
