...
function ClubsPageContent() {
  ...
  const {
    ...
    hasMore,
    sentinelRef,
  } = useInfiniteScroll<Club>(clubListRef, "/api/clubs", process.env.NEXT_PUBLIC_SITE_URL ?? null, { queryParams });
  ...
  useEffect(() => {
    if (!loaded && !isLoading && clubs.length > 0) {
      ...
      const match = clubIdFromUrl
        ...
        : null;
      setSelectedClub(match ?? clubs[0] ?? null);
  ...
  useEffect(() => {
    ...
    if (!selectionStillValid) {
      setSelectedClub(clubs.length > 0 ? (clubs[0] ?? null) : null);
    }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (nextIndex !== currentIndex && clubs[nextIndex]) {
